...
export const api = {
  ...
  songMetadataCandidates: (id: number, scope: 'path' | 'online' | 'all' = 'all') => request<MetadataCandidate[] | null>(`/api/songs/${id}/metadata-candidates?scope=${encodeURIComponent(scope)}`).then(arrayOrEmpty),
  ...
  albumMetadataCandidates: (id: number, scope: 'path' | 'online' | 'all' = 'all') => request<MetadataCandidate[] | null>(`/api/albums/${id}/metadata-candidates?scope=${encodeURIComponent(scope)}`).then(arrayOrEmpty),
