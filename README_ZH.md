...
百灵是一个面向个人音乐库的自托管 Web 音乐播放器。把 NAS、家用服务器、桌面机器或容器卷里的音乐目录交给它，就可以在浏览器里扫描、浏览、修正、播放、离线缓存和分享自己的曲库。

它优先照顾本地大曲库：FLAC/WAV/Hi-Res 专辑、CUE 整轨、老格式、混乱 Tag、缺失歌词、错误封面，以及多年攒下来的目录结构。围绕听歌这件事，它也提供移动端播放、浏览器离线缓存、在线电台、DLNA 设备投放、Subsonic 兼容客户端、ListenBrainz/Last.fm 同步和公开分享链接。
...
- 本地音乐已经多到文件浏览器和共享文件夹不好用了。
- 想在 NAS、家用服务器、VPS 或常开的桌面机器上搭一个私有音乐站。
- 曲库里有 FLAC/WAV/Hi-Res 专辑、CUE 整轨和需要谨慎处理的老格式。
- Tag、封面、歌手名或歌词不够干净，希望边听边修。
- 平时用桌面和手机浏览器听歌，偶尔需要离线，也想投放到 DLNA 设备或接入 Subsonic/Navidrome 兼容客户端。
- 想把一首歌、一张专辑、一个歌手或一份歌单分享给别人，但不想给对方开账号。
...
首次打开时，在 Web 界面创建管理员账号。如果要无人值守初始化，可以在第一次启动前传入：
...
默认情况下，百灵会把应用数据和上传音乐保存在 `lark_data` Docker volume 中。如果你的音乐目录已经暴露到容器内部，把 `LARK_LIBRARY_DIR` 设置成那个路径，再到 Web 界面里扫描即可。
...
- 歌曲、专辑、歌手和歌单列表都支持分页；专辑艺人筛选、歌手首字母筛选、文件夹浏览和来源标签页适合处理较大的不规则曲库。
- 专辑、歌手和歌单详情页支持一键播放。
...
- 可以归一部分异常歌手名，保存歌手首字母，并支持 A-Z / # 筛选。
- 优先读取音频文件内嵌歌词；缺少内嵌歌词时，再自动匹配在线歌词。
- 自动匹配不准时，可以手动选择歌词候选。LRC 解析支持 offset 标签、一行多个时间戳、毫秒精度，以及同一时间戳下的原文/翻译分组。
...
- 桌面首页播放样式包括黑胶唱机、磁带卡座、iPod、蓝调音箱、唱片封套、锤子唱机、复古唱机和奔跑的小猫。
- 移动端播放样式包括精密音频、复古唱机、独立蓝调、iPod、柔光唱片、舞台玻璃、蓝色光环和锤子经典。
- 底部播放器提供播放队列、播放模式、音量、进度、收藏、定时关闭和全屏歌词入口。
- 定时关闭支持按时长、按播放首数，或当前专辑结束后停止。
- 播放队列、来源上下文、续播位置和播放历史可以跨会话保留；跨设备继续收听会恢复保存的队列，而不是只剩最近一首。
- 播放历史也可以按设备隔离，让手机、平板和浏览器保留各自的播放轨迹。
- EQ 预设适用于本地音乐、网络曲库和电台播放；可选界面音效会给播放、收藏和分享动作提供轻量反馈。
- `播放到其他设备` 可以把当前本地曲库歌曲和百灵队列投放到同一局域网内发现的 DLNA 渲染器，例如电视、功放、音箱或机顶盒。如果设备无法访问百灵发出的媒体 URL，请在站点设置里把 **媒体 Base URL** 设置为后端在局域网内可访问的地址，例如 `http://192.168.1.8:8080`。
- 布局适配桌面侧栏、Pad 图标侧栏和手机底部导航。
- 歌曲可以准备为浏览器侧离线缓存，并显示缓存状态和占用空间；也可以把听过的歌自动缓存到本机，断网或开启离线模式时优先播放缓存音频。
...
- 可选 DLNA 曲库暴露可以让电视、功放、音箱和第三方 DLNA 客户端在局域网内发现百灵，并浏览歌曲、专辑、歌手、歌单和文件夹。它是独立于投放的管理员设置，默认关闭。
...
- 设置项覆盖语言、主题、曲库路径、目录状态、目录监控、诊断开关、字体上传、歌词字体、DLNA 投放/曲库暴露和转码策略。
