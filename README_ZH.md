...
- **20 套主题，色板化选择。** 主题入口从原来的下拉框换成「设置 → 站点」里的 20 格色板，每格都是实时预览，深/浅色用小圆点区分，当前主题会显示强调色描边。20 套主题是 5 套原创深色（深空极夜、琥珀胶片、霓虹珊瑚、冰川极光、碳黑燃绿）、5 套原创浅色（奶白陶瓷、燕麦拿铁、薄荷苏打、樱花宣纸、黄昏琥珀）、以及 10 套致敬（Apple Music / Spotify / 网易云 / Winamp / Foobar2000 各深浅一版）。
- **浅色主题对比度修复。** 5 套原创浅色主题重新调过暗色辅色 `--muted`，正文文字对比度从 3.4:1 提升到 4.5:1 以上。致敬版浅色主题同步对齐。
- **歌词匹配失败兜底。** 在线歌词如果和当前歌曲不匹配，全屏歌词顶部会显示「匹配可能不准」徽章，提示用户换一首或确认歌名/艺人。
- **全屏歌词布局。** 长歌曲名不再顶到右侧操作按钮。
- **曲库行质量列。** `2024 · MP3 · 44.1kHz` 这一列不再被截断，鼠标悬停还能看到完整 title。
- **空状态统一。** 新增通用 `EmptyState` 组件，收藏夹等页面统一使用。
- **专辑 / 歌手页标题。** 详情页顶部加面包屑标题（例如「专辑 / Driftwood」），不再需要靠前一个页面记忆。
- **打磨。** 曲库顶部留白收紧；主内容区底部补上播放器高度的间距；设置 tab 选中态有强调色圆点 + 投影；主题切换加了 0.32s 的平滑过渡；少量占位文案重写得更自然。

完整 diff 可以看 `git log v0.7.17..v0.7.18`。
...
- 主题按你提供的播放器方案实现：共 20 套——5 套原创深色（深空极夜、琥珀胶片、霓虹珊瑚、冰川极光、碳黑燃绿）、5 套原创浅色（奶白陶瓷、燕麦拿铁、薄荷苏打、樱花宣纸、黄昏琥珀），以及 10 套致敬（Apple Music、Spotify、网易云、Winamp、Foobar2000 各深浅一版）。
- 主题入口只放在「设置 → 站点」里的 20 格色板（带实时预览），主界面保持简洁。
