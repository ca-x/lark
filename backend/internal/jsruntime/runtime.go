...
type JSEnv struct {
	...
	bridgeMu       sync.RWMutex       // 保护 bridgeCallback 的安装与快照
...
func registerBridgeFunctions(vm *quickjs.VM, env *JSEnv) error {
	...
	// __go_bridge_sync(action, data) -> error message
	//
	// SongLoft 的 provider/event 注册 API 是同步 void。只允许这些纯内存注册动作
	// 同步执行，确保插件 onInit 返回时宿主已经能观察到注册状态。网络、数据库、
	// 文件和进程操作仍必须走下面的异步桥，避免在持有 QuickJS VM 锁时阻塞。
	if err := vm.RegisterFunc("__go_bridge_sync", func(action, data string) string {
		if !isSynchronousBridgeAction(action) {
			return "synchronous bridge action is not allowed: " + action
		}
		env.bridgeMu.RLock()
		cb := env.bridgeCallback
		env.bridgeMu.RUnlock()
		if cb == nil {
			return "no bridge callback registered"
		}
		_, err := cb(action, data)
		if err != nil {
			slog.Warn("__go_bridge_sync error", "envID", env.envID, "action", action, "error", err)
			return err.Error()
		}
		return ""
	}, false); err != nil {
		return fmt.Errorf("register __go_bridge_sync: %w", err)
	}
...
func isSynchronousBridgeAction(action string) bool {
	switch action {
	case "plugin.registerPlayEvent",
		"plugin.unregisterPlayEvent",
		"plugin.registerLyricProvider",
		"plugin.unregisterLyricProvider",
		"plugin.registerCoverProvider",
		"plugin.unregisterCoverProvider":
		return true
	default:
		return false
	}
...
// 导出提交 issue，故落日志前先脱敏（导出端点仍应再做一次全局脱敏）。
...
func redactHeaderJSONForLog(headersJSON string) any {
...
func doHTTPRequest(url, method, headersJSON, bodyHex string) string {
	slog.Debug("doHTTPRequest", "url", url, "method", method, "headers", redactHeaderJSONForLog(headersJSON))
