...
func (_q *AlbumQuery) QuerySongs() *SongQuery {
...
func (_q *AlbumQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AlbumQuery) FirstIDX(ctx context.Context) int {
...
func (_q *AlbumQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AlbumQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *AlbumQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *AlbumQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AlbumQuery) WithSongs(opts ...func(*SongQuery)) *AlbumQuery {
	query := (&SongClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AlbumQuery) loadSongs(ctx context.Context, query *SongQuery, nodes []*Album, init func(*Album), assign func(*Album, *Song)) error {
	...
	query.Where(predicate.Song(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(album.SongsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.album_songs
		if fk == nil {
			return fmt.Errorf(`foreign-key "album_songs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
