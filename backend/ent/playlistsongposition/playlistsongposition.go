// Code generated by ent, DO NOT EDIT.

package playlistsongposition

import (
	"entgo.io/ent/dialect/sql"
)

const (
	// Label holds the string label denoting the playlistsongposition type in the database.
	Label = "playlist_song_position"
	// FieldID holds the string denoting the id field in the database.
	FieldID = "id"
	// FieldPlaylistID holds the string denoting the playlist_id field in the database.
	FieldPlaylistID = "playlist_id"
	// FieldSongID holds the string denoting the song_id field in the database.
	FieldSongID = "song_id"
	// FieldPosition holds the string denoting the position field in the database.
	FieldPosition = "position"
	// Table holds the table name of the playlistsongposition in the database.
	Table = "playlist_song_positions"
)

// Columns holds all SQL columns for playlistsongposition fields.
var Columns = []string{
	FieldID,
	FieldPlaylistID,
	FieldSongID,
	FieldPosition,
}

// ValidColumn reports if the column name is valid (part of the table columns).
func ValidColumn(column string) bool {
	for i := range Columns {
		if column == Columns[i] {
			return true
		}
	}
	return false
}

// OrderOption defines the ordering options for the PlaylistSongPosition queries.
type OrderOption func(*sql.Selector)

// ByID orders the results by the id field.
func ByID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldID, opts...).ToFunc()
}

// ByPlaylistID orders the results by the playlist_id field.
func ByPlaylistID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldPlaylistID, opts...).ToFunc()
}

// BySongID orders the results by the song_id field.
func BySongID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldSongID, opts...).ToFunc()
}

// ByPosition orders the results by the position field.
func ByPosition(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldPosition, opts...).ToFunc()
}
